...
import { escapeHtml, singleLine } from "@/lib/security/escape-html";
...
function renderInviteHtml(input: { orgName: string; inviterEmail: string; roleLabel: string; acceptUrl: string }, t: Strings) {
  const intro = fill(t.inviteEmailIntroText, {
    inviter: escapeHtml(input.inviterEmail),
    org: escapeHtml(input.orgName),
    role: escapeHtml(input.roleLabel),
  });
  return `
    <div style="font-family: -apple-system, Helvetica, Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px 16px; color: #1a1a1a;">
      ...
      <p style="margin: 28px 0;">
        <a href="${escapeHtml(input.acceptUrl)}" style="display: inline-block; background: #d99e39; color: #14261c; font-weight: 600; font-size: 14px; padding: 10px 20px; border-radius: 8px; text-decoration: none;">${t.inviteEmailButton}</a>
      </p>
...
export async function sendOrgInvite(input: {
  ...
  await send({
    to: [input.to],
    subject: singleLine(fill(t.inviteEmailSubject, { org: input.orgName, inviter: input.inviterEmail })),
